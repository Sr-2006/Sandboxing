...
from phase2_vector_memory import get_collection_name
...
def search_historical_incidents(error_log: str, top_k: int = 2, sandbox: str = "production") -> str:
    ...
    Pass sandbox="shadow" to query shadow incident memory, or "production" for production memory.
    """
    collection_name = get_collection_name(sandbox == "shadow")
    print(f"[MCP] Querying collection: {collection_name}")
    target_collection = chroma_client.get_or_create_collection(name=collection_name)
