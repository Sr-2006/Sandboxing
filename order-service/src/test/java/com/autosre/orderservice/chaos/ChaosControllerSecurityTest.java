...
public class ChaosControllerSecurityTest {
    ...
    @Test
    @DisplayName("Clear endpoints require valid token")
    void testClearRequiresToken() {
        ...
        assertThrows(ResponseStatusException.class, () -> {
            controller.clearDeadlock(null);
        });

        ResponseEntity<?> dResponse = controller.clearDeadlock("test-chaos-token");
        assertEquals(HttpStatus.OK, dResponse.getStatusCode());
    }
}
